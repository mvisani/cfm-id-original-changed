	...
	for( ; testit != test_cases.end(); ++testit ){
		...
		if( fv->getNumSetFeatures() != 2 ){
			...
			if(fv->getFeature(1) != testit->second ){
				std::cout << "Unexpected broken bond type: expecting " << testit->second << " but found " << fv->getFeature(
                        1) << std::endl;
				pass = false;
			}
		}
		...
		if( fv->getNumSetFeatures() != testit->second.size()+1 ){
			...
			for( int idx = 1; it != testit->second.end(); ++it, idx++ ){
				if(fv->getFeature(idx) != *it ){
					std::cout << "Unexpected feature in neighbour bond type: expecting " << *it << " but found " << fv->getFeature(
                            idx) << std::endl;
					pass = false;
				}
			}
	...
	for( ; testit != test_cases.end(); ++testit ){
		...
		if( fv->getNumSetFeatures() != 3 ){
			...
			if(fv->getFeature(1) != testit->second.first + 1  ){				//Ion
				std::cout << "Unexpected Ion Root Atom Feature: expecting " << testit->second.first + 1 << " but found " << fv->getFeature(
                        1) << std::endl;
				pass = false;
			}
	...
	for( ; testit != test_cases.end(); ++testit ){
		...
		if( fv->getNumSetFeatures() != testit->expected_output.size()+1 ){
			...
			for( int idx = 1; it != testit->expected_output.end(); ++it, idx++ ){
				if(fv->getFeature(idx) != *it ){
					std::cout << "Unexpected Ionic Feature at idx " << idx << ": expecting " << *it << " but found " << fv->getFeature(
                            idx) << std::endl;
					pass = false;
				}
			}
...
void FeaturesTestMetlinExample::runTest(){
	...
	mol.computeFragmentGraphAndReplaceMolsWithFVs(&fc, false);
	//mol.computeFeatureVectors(&fc, 0, false);
