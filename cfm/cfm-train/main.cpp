...
void trainSingleEnergyCFM(std::string &param_filename, config_t &cfg,
                          FeatureCalculator &fc, std::string &status_filename,
                          int group, std::vector<MolData> &data,
                          int start_energy, int no_train, int start_repeat);
...
int main(int argc, char *argv[]) {
  ...
  std::string
      peakfile_dir_or_msp; // MSP file or Directory containing the peak files
                           // for each molecule (in format <id>.txt)
  ...
  general_options.add_options()("help,h", "Help message")(
      ...
      "then id smiles_or_inchi cross_validation_group on each line after "
      "that.")(
      "feature_filename,f",
      po::value<std::string>(&feature_filename)->required(),
      ...
      "peakfile_dir_or_msp,p",
      po::value<std::string>(&peakfile_dir_or_msp)->required(),
      ...
      "Directory containing files with spectra. Each file should be called "
      "<id>.txt, "
      "where <id> is the id specified in the input file, and contains a list "
      "of peaks "
      "'mass intensity' on each line, with either 'low','med' and 'high' lines "
      "beginning "
      ...
      "number for n-fold cross validation, if not provide, noy cross "
      "validation will be applied. "
      "If group number is set, this number will be igroned.")(
      "tmp_data_folder,t",
      po::value<std::string>(&data_folder)->default_value("tmp_data"),
      "Name of folder to write tmp data for training. If not specified will "
      "write to tmp_data")(
      "log_file,l",
      po::value<std::string>(&status_filename)->default_value("status.log"),
      "Name of file to write logging information as the program runs."
      "If not specified will write to status.log<group>, or status.log if no "
      "group is specified")("start_energy,e",
                            po::value<int>(&start_energy)->default_value(0),
      ...
      "Set to starting repeat if want to start training part way through "
      "(default 0)")(
      "fv_fragment_graphs_folder,a",
      po::value<std::string>(&fv_fragment_graphs_folder)->default_value(""),
      "Name of folder to write and read fragement cache data for training. If "
      "not specified will write to "
      "tmp_data/fv_fragment_graphs_folder")(
      "no_train", po::value<bool>(&no_train)->default_value(false),
  ...
  boost::filesystem::path fv_fragment_graphs_folder_path(
      fv_fragment_graphs_folder);
  ...
  while (mit != data.end()) {
    try {
      ...
      std::string fv_filename = fv_fragment_graphs_folder + "/" +
                                boost::lexical_cast<std::string>(mit->getId()) +
                                "_graph.fg";
      ...
      if (boost::filesystem::exists(fv_filename)) {
        ...
        std::ifstream fv_ifs;
        fv_ifs = std::ifstream(fv_filename.c_str(),
        ...
        after = time(nullptr);
        eout << "ID: " << mit->getId()
             << " is Done. Time Elaspsed = " << (after - before) << " Seconds ";
        eout << " Num Frag = " << mit->getNumFragments();
        eout << " Num Trans = " << mit->getNumTransitions() << std::endl;
        eout.close();
  ...
  std::cout << "Got : " << success_count << " successfully computed. "
  ...
  if (peakfile_dir_or_msp.substr(peakfile_dir_or_msp.size() - 4, 4) == ".msp") {
    ...
    // some_file_name<P>.msp, where <P> gets replaced with the processor rank
    // that uses that file
    pre_id = peakfile_dir_or_msp.substr(0, peakfile_dir_or_msp.size() - 4);
    if (pre_id.substr(pre_id.size() - 3, 3) == "<P>") {
      peakfile_dir_or_msp.replace(peakfile_dir_or_msp.size() - 7, 3,
    ...
    if (!no_train) {
      if (spectra_in_msp)
        ...
        std::string spec_file =
            peakfile_dir_or_msp + "/" + mit.getId() + ".txt";
      ...
      std::ofstream eout;
      eout.open(status_filename.c_str(), std::fstream::out | std::fstream::app);
      eout << "ID: " << mit.getId() << " Remove Peaks With No Fragment. "
           << remove_msg << std::endl;
      eout.close();
    }
  }
  ...
  for (int group = min_group; group < max_group; group++) {
    ...
    std::cout << "Running EM to train parameters for Group " << group
              << std::endl;
    ...
    if (!no_train)
      trainSingleEnergyCFM(param_filename, cfg, fc, status_filename, group,
                           data, start_energy, no_train, start_repeat);
    ...
    if (spectra_in_msp) {
      std::string proc_group_str =
          "P" + boost::lexical_cast<std::string>(mpi_rank) + "G" +
          boost::lexical_cast<std::string>(group);
      std::string enum_msp_filename =
          data_folder + "/enumerated_output/especs_" + proc_group_str + ".msp";
      std::string pred_msp_filename =
          data_folder + "/predicted_output/pspecs_" + proc_group_str + ".msp";
    ...
    std::cout << "Generating Peak Predictions for Group " << group << " from "
    ...
    for (auto mit = data.begin(); mit != data.end(); ++mit) {
      if (mit->getGroup() != group)
        continue;
      if (!mit->hasComputedGraph())
      ...
      if (spectra_in_msp)
        ...
        std::string spectra_filename =
            data_folder + "/predicted_output/" + mit->getId() + ".log";
...
void trainSingleEnergyCFM(std::string &param_filename, config_t &cfg,
  ...
  for (int energy = 0; energy < cfg.spectrum_depths.size(); energy++) {
    ...
    std::string eparam_filename =
        param_filename + boost::lexical_cast<std::string>(energy) + "e";
    std::string prev_eparam_filename =
        param_filename + boost::lexical_cast<std::string>(energy - 1) + "e";

    config_t se_cfg;
    initSingleEnergyConfig(se_cfg, cfg, energy);

    if (energy >= start_energy && !no_train) {
      // Run EM multiple times with random restarts, taking the final one with
      // the best Q
      double prev_Q = -DBL_MAX;
      if (energy > start_energy)
        start_repeat = 0;
      if (start_repeat >
          0) { // Too messy to compute previous best Q...just print a warning
        std::cout << "Warning: best Q for previous repeats unknown, may not "
                     "pick up best params - check manually!"
                  << std::endl;
      }
      for (int repeat = start_repeat; repeat < se_cfg.num_em_restarts;
           repeat++) {

        EmModel *em;
        std::string repeat_filename =
            eparam_filename + boost::lexical_cast<std::string>(repeat);
        ...
        if (!boost::filesystem::exists(repeat_filename))
          ...
          repeat_filename =
              prev_eparam_filename + boost::lexical_cast<std::string>(repeat);
        if (cfg.theta_function == NEURAL_NET_THETA_FUNCTION)
        ...
        if (Q > prev_Q) {
          ...
          std::cout << "Found better Q! Write to " << eparam_filename
                    << std::endl;
