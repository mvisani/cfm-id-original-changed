...
struct NNParamTestFixture {
  NNParamTestFixture() {
    ...
    std::copy(weights.begin(), weights.end(),
              std::back_inserter(weights_for_all_energy));
    std::copy(weights.begin(), weights.end(),
              std::back_inserter(weights_for_all_energy));
...
BOOST_DATA_TEST_CASE(UnweightedGradientsTest, bdata::make(energies), energy) {
  ...
  fvs[0] = &fv1;
  fvs[1] = &fv2;
  param->computeUnweightedGradients(unweighted_grads, used_idxs, fvs, deltasA,
                                    deltasB, a_values);

  // Check the values
  float x1 = exp(theta1) / rho_denom, x2 = exp(theta2) / rho_denom;
  float exp_unweighted_1[] = {-5 + 5 * x1,
                              0,
                              -5 + 5 * x1,
                              0,
                              0,
  ...
  float exp_unweighted_2[] = {5 * x1,
                              ...
                              +1 - 3 * x1 - x2,
                              -7 + 13 * x1 + 7 * x2};
  float exp_unweighted_persist[] = {5 * x1,
                                    0,
                                    5 * x1,
                                    0,
                                    0,
  ...
  for (int i = 0; i < 37; i++) {
    BOOST_CHECK_CLOSE_FRACTION(exp_unweighted_1[i], unweighted_grads[0][i],
                               tol);
    BOOST_CHECK_CLOSE_FRACTION(exp_unweighted_2[i], unweighted_grads[1][i],
                               tol);
    BOOST_CHECK_CLOSE_FRACTION(exp_unweighted_persist[i],
                               unweighted_grads[2][i], tol);
  }

  std::set<int> expected_used_idxs = {0,  2,  3,  5,  6,  8,  9,  11,
...
BOOST_AUTO_TEST_CASE(GradientTest) {
  ...
  suft.values[0][3] = 0.0;
  suft.values[0][4] = 0.0;
  // suft.values[0][N] = 0.9; suft.values[0][N+1] = 0.05; suft.values[0][N+2] =
  // 0.05; suft.values[0][N+3] = 0.0; suft.values[0][N+4] = 0.0;
  // suft.values[0][2*N] = 0.08; suft.values[0][2*N+1] = 0.9;
  // suft.values[0][2*N+2] = 0.02; suft.values[0][2*N+3] = 0.0;
  // suft.values[0][2*N+4] = 0.0;
  ...
  int mol_idx = 0;
  double Q_only =
      em.computeLogLikelihoodLoss(mol_idx, moldata, suft, energy_level);
  ...
  double expected_Q = (0.2) * (theta1 - log(rho_denom)) +
                      (0.3) * (theta2 - log(rho_denom)) +
                      (0.5) * (-log(rho_denom));

  // Check the gradients
  double exp_unweighted_1[] = {-5 + 5 * x1,
                               0,
                               -5 + 5 * x1,
                               0,
                               0,
  ...
  double exp_unweighted_2[] = {5 * x1,
                               ...
                               +1 - 3 * x1 - x2,
                               -7 + 13 * x1 + 7 * x2};
  double exp_unweighted_persist[] = {5 * x1,
                                     0,
                                     5 * x1,
                                     0,
                                     0,
  ...
  for (unsigned int i = 0; i < param->getNumWeightsPerEnergyLevel(); i++)
    BOOST_CHECK_CLOSE_FRACTION(grads[i],
                               (0.2 * exp_unweighted_1[i] +
                                0.3 * exp_unweighted_2[i] +
                                0.5 * exp_unweighted_persist[i]),
                               tol);

  // Check used Index
  std::vector<unsigned int> expected_idxs = {
...
BOOST_AUTO_TEST_CASE(NNParamsTestBiasIndexes) {
  ...
  std::vector<int> act_ids{
      RELU_NN_ACTIVATION_FUNCTION, RELU_NN_ACTIVATION_FUNCTION,
...
BOOST_AUTO_TEST_CASE(NNParamsTestDropout) {
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(dropout_probs.begin(), dropout_probs.end(),
                                dropout_prob_ptr->begin(),
                                dropout_prob_ptr->end());
  ...
  for (int trail = 0; trail < 10; ++trail) {
    ...
    for (auto h_layer_idx = 0; h_layer_idx < hlayer_numnodes.size();
         ++h_layer_idx) {
...
BOOST_AUTO_TEST_CASE(SaveAndLoadParamFileTest) {
  ...
  BOOST_CHECK_EQUAL(param.getNumEnergyLevels(),
                    param_load.getNumEnergyLevels());
  ...
  for (int i = 0; i < param.getNumWeights(); i++)
    BOOST_CHECK_CLOSE_FRACTION(param.getWeightAtIdx(i),
                               param_load.getWeightAtIdx(i), tol);
