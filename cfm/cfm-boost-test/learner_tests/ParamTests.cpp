...
BOOST_AUTO_TEST_CASE(GradientTest) {
	...
	// Set some arbitrary parameter weights (only used in the regularizer term and
	// for sizing)
	std::vector<std::string> fnames;
	...
	if (boost::filesystem::exists(param_filename)) boost::filesystem::remove(param_filename);
	...
	// get used flags
	// em.computeAndAccumulateGradient(&grads[0], 0, moldata, suft, true,
	// used_idxs, 0, 0);
	...
	for (unsigned int i = 0; i < 6; i++) { BOOST_CHECK_CLOSE_FRACTION(grads[i], expected_grads[i], tol); }
}
