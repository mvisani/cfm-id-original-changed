...
void Sgd::adjustWeights(std::vector<double> &grads, std::set<unsigned int> &used_idxs, boost::shared_ptr<Param> param) {

	for (auto &used_idx : used_idxs) (*(param->getWeightsPtr()))[used_idx] += learning_rate * grads[used_idx];
}
...
void Momentum::adjustWeights(std::vector<double> &grads, std::set<unsigned int> &used_idxs,
...
void Adam::adjustWeights(std::vector<double> &grads, std::set<unsigned int> &used_idxs,
                         boost::shared_ptr<Param> param) {
...
void AdamW::adjustWeights(std::vector<double> &grads, std::set<unsigned int> &used_idxs,
                          boost::shared_ptr<Param> param) {
	...
	for (auto &used_idx : used_idxs) {
		...
		if (std::find(bias_index.begin(), bias_index.end(), used_idx) != bias_index.end())
			(*(param->getWeightsPtr()))[used_idx] =
			    (1.0 - w) * (*(param->getWeightsPtr()))[used_idx] + learning_rate * m_hat / (std::sqrt(v_hat) + eps);
		else
			(*(param->getWeightsPtr()))[used_idx] =
			    (*(param->getWeightsPtr()))[used_idx] + learning_rate * m_hat / (std::sqrt(v_hat) + eps);
	}
...
void AdaDelta::adjustWeights(std::vector<double> &grads, std::set<unsigned int> &used_idxs,
                             boost::shared_ptr<Param> param) {
...
void AdaBelief::adjustWeights(std::vector<double> &grads, std::set<unsigned int> &used_idxs,
                              boost::shared_ptr<Param> param) {
