...
EmModel::computeMetrics(const Spectrum *p, const Spectrum *q) {
  double dice_out, dp_out, precision_out, recall_out;

  Comparator *dice_cmp = new Dice(cfg->ppm_mass_tol, cfg->abs_mass_tol);
  Comparator *dotproduct_cmp =
      new DotProduct(cfg->ppm_mass_tol, cfg->abs_mass_tol);
...
void EmModel::recordSufficientStatistics(suft_counts_t &suft, int molidx,
  ...
  int depth = cfg->model_depth > moldata->getFGHeight()
                  ? cfg->model_depth
                  : moldata->getFGHeight();
...
void EmModel::collectUsedIdx(MolData &mol_data,
  ...
  for (auto frag_trans_map = mol_data.getFromIdTMap()->begin();
       frag_trans_map != mol_data.getFromIdTMap()->end(); ++frag_trans_map) {
    for (auto trans_id : *frag_trans_map) {
      const FeatureVector *fv = mol_data.getFeatureVectorForIdx(trans_id);
      for (auto fv_it = fv->getFeatureBegin(); fv_it != fv->getFeatureEnd();
           ++fv_it)
...
double EmModel::updateParametersGradientAscent(std::vector<MolData> &data,
                                               suft_counts_t &suft,
  ...
  if (this->used_idxs.empty()) {
    ...
    if (cfg->collected_all_used_idx) {
      ...
      for (int i = 0; i < param->getNumWeightsPerEnergyLevel(); ++i) {
        this->used_idxs.insert(grad_offset + i);
      }
    }
...
#pragma omp critical
    ...
    std::cout << "[M-Step][T+" << getTimeDifferenceStr(start_time, after)
              << "s]Collect Used Index, Time Used: "
              << getTimeDifferenceStr(before, after) + "s" << std::endl;
  }
  ...
  while (iter++ < max_iteration &&
         ga_no_progress_count <= cfg->ga_no_progress_count) {
    auto before = std::chrono::system_clock::now();
    if (iter > 1) {
      prev_loss = loss;
    }
...
#pragma omp parallel for reduction(+ : num_trans)
      for (int molidx = 0; molidx < data.size(); ++molidx) {
        ...
        if (minibatch_flags[molidx] == batch_idx &&
            mol_it.getGroup() != validation_group) {
...
#pragma omp critical
    ...
    std::cout << iter << ".[T+" << getTimeDifferenceStr(start_time, after)
              << "s]" << "Loss=" << loss << " Prev=" << prev_loss
              << " Best=" << prev_best_loss
              << " Change Rate=" << loss_change_rate
              << " Time Escaped=" << getTimeDifferenceStr(before, after)
              << "s Learning Rate=" << learning_rate;
    ...
    // we are doing ga, save best weight some where and check if we have getting
    // better
  ...
  if (iter == cfg->ga_max_iterations)
    ...
    std::cout << "[M-Step]Gradient ascent converged after " << iter - 1
              << " iterations" << std::endl;
  delete solver;
...
double EmModel::trainModel(std::vector<MolData> &molDataSet, int group,
                           std::string &out_param_filename, int energy_level) {
  unused_zeroed = 0;
  int iter = 0;
  ...
  double max_pob_dice = 0.0, max_pob_dp = 0.0, max_pob_precision = 0.0,
         max_pob_recall = 0.0;

#pragma omp parallel for reduction(+ : max_pob_dice, max_pob_dp,               \
                                       max_pob_precision, max_pob_recall,      \
                                       num_training_mols, num_val_mols)
  for (auto &mol_data : molDataSet) {
#pragma omp critial
    if (mol_data.hasEmptySpectrum(energy_level) &&
        mol_data.getGroup() != validation_group)
      std::cout << "Warning: No peaks with explanatory fragment found for "
                << mol_data.getId() << ", ignoring this input molecule."
                << std::endl;

    // compute reachable mectirs with curret fgs
    if (mol_data.getGroup() != validation_group) {
      auto [internal_prob_dice, internal_pob_db, internal_precision,
            internal_recall] =
          computeMetrics(mol_data.getOrigSpectrum(energy_level),
                         mol_data.getSpectrum(energy_level));
  ...
  pre_train_str +=
      "[Pre-Train][Max Possible Metrics for Current Fragmentation Setting]\n";
  pre_train_str +=
      "Dice=" + std::to_string(max_pob_dice / num_training_mols) +
      " DotProduct=" + std::to_string(max_pob_dp / num_training_mols) +
      " Precision=" +
      std::to_string(max_pob_precision / num_training_mols / 100.0f) +
...
#pragma omp parallel for
    for (int molidx = 0; molidx < molDataSet.size(); ++molidx) {
      ...
      if (!mol.hasComputedGraph()) {
        continue;
      }
      if (mol.hasEmptySpectrum(energy_level)) {
        continue;
      }
      if (mol.getGroup() == validation_group &&
          cfg->disable_cross_val_metrics) {
        continue;
      }
      computeThetas(&mol);
...
#pragma omp critical // since we are updating shared suft we need to make sure
                     // we are not writing to the same memory location
    ...
    loss = updateParametersGradientAscent(molDataSet, suft, learning_rate,
                                          sampling_method, energy_level);
    ...
    int molidx = 0;
    double train_dice = 0.0, train_dp = 0.0, train_precision = 0.0,
           train_recall = 0.0;
    double val_dice = 0.0, val_dp = 0.0, val_precision = 0.0, val_recall = 0.0;

    double pruned_train_dice = 0.0, pruned_train_dp = 0.0,
           pruned_train_precision = 0.0, pruned_train_recall = 0.0;

#pragma omp parallel for reduction(                                            \
        + : val_q, train_dice, train_dp, train_precision, train_recall,        \
            val_dice, val_dp, val_precision, val_recall, pruned_train_dice,    \
            pruned_train_dp, pruned_train_precision, pruned_train_recall)
    for (int molidx = 0; molidx < molDataSet.size(); ++molidx) {
      auto &mol_it = molDataSet[molidx];
      // run preduiction
      mol_it.computePredictedSpectra(
          *param, false, energy_level, cfg->default_predicted_peak_min,
      ...
      if (mol_it.getGroup() == validation_group &&
        ...
        auto [val_dice_internal, val_dp_internal, val_precision_internal,
              val_recall_internal] =
            computeMetrics(mol_it.getOrigSpectrum(energy_level),
      ...
      } else if (mol_it.getGroup() != validation_group &&
                 !cfg->disable_training_metrics) {
        auto [train_dice_internal, train_dp_internal, train_precision_internal,
              train_recall_internal] =
            computeMetrics(mol_it.getOrigSpectrum(energy_level),
                           mol_it.getPredictedSpectrum(energy_level));
        ...
        auto [pruned_train_dice_internal, pruned_train_dp_internal,
              pruned_train_precision_internal, pruned_train_recall_internal] =
            computeMetrics(mol_it.getSpectrum(energy_level),
                           mol_it.getPredictedSpectrum(energy_level));
    ...
    after = std::chrono::system_clock::now();
    std::string q_time_msg = "[M-Step][T+" +
                             getTimeDifferenceStr(start_time, after) +
                             "s]Finished loss compute: Time Elapsed = " +
    ...
    qdif_str += "Total=" + std::to_string(loss) +
                " Mean=" + std::to_string(loss / num_training_mols);

    if (prev_loss != -DBL_MAX)
      qdif_str += " Change Ratio= " + std::to_string(loss_change_rate) +
                  " Prev=" + std::to_string(prev_loss);
    ...
    if (!cfg->disable_training_metrics) {
      ...
      qdif_str +=
          "Dice=" + std::to_string(train_dice / num_training_mols) +
          " DotProduct=" + std::to_string(train_dp / num_training_mols) +
          " Precision=" +
          std::to_string(train_precision / num_training_mols / 100.0f) +
          " Recall=" +
          std::to_string(train_recall / num_training_mols / 100.0f);

      qdif_str += "\n[M-Step][Training Metric (Pruned)] ";
      qdif_str +=
          "Dice=" + std::to_string(pruned_train_dice / num_training_mols) +
          " DotProduct=" + std::to_string(pruned_train_dp / num_training_mols) +
          " Precision=" +
          std::to_string(pruned_train_precision / num_training_mols / 100.0f) +
          " Recall=" +
          std::to_string(pruned_train_recall / num_training_mols / 100.0f);
    }

    if (!cfg->disable_cross_val_metrics) {
      qdif_str += "\n[M-Step][Validation Metric]        ";
      qdif_str +=
          "Dice=" + std::to_string(val_dice / num_val_mols) +
          " DotProduct=" + std::to_string(val_dp / num_val_mols) +
          " Precision=" +
          std::to_string(val_precision / num_val_mols / 100.0f) + " Recall=" +=
          std::to_string(val_recall / num_val_mols / 100.0f);
    }
    ...
    if (best_loss < loss) {
      ...
      std::string progress_str =
          "[M-Step] Found Better Q: " + std::to_string(best_loss) +
          " Write to File: " + out_param_filename;
      std::cout << progress_str << std::endl;
      writeParamsToFile(out_param_filename);
    }
    writeParamsToFile(iter_out_param_filename);
    updateEmTrainingParams(loss_change_rate, learning_rate, sampling_method,
                           em_no_progress_count);
    // std::cerr << em_no_progress_count << std::endl;
    if (em_no_progress_count >= cfg->em_no_progress_count) {

      std::cout << "EM Stopped after " + std::to_string(em_no_progress_count) +
                       " No Progress Iterations"
                << std::endl;
      ;
      std::cout << "EM Converged after " + std::to_string(iter) + " iterations"
                << std::endl;
  ...
  if (iter >= cfg->em_max_iterations) {
    std::cout << "Warning: EM still converging after " + std::to_string(iter) +
                     " iterations."
              << std::endl;
  }
  return best_loss;
...
float EmModel::getTimeDifference(
...
std::string EmModel::getTimeDifferenceStr(
    const std::chrono::system_clock::time_point &before,
    const std::chrono::system_clock::time_point &after) {
...
void EmModel::updateEmTrainingParams(double loss_change_rate,
                                     float &learning_rate, int &sampling_method,
  ...
  if (loss_change_rate < cfg->em_converge_thresh) {
    if (cfg->ga_reset_sampling && sampling_method != cfg->ga_sampling_method2) {
      ...
      std::cout << "Switched to sampling method: " +
                       std::to_string(sampling_method)
                << std::endl;
...
double EmModel::computeAndSyncLoss(std::vector<MolData> &data,
                                   suft_counts_t &suft, unsigned int energy) {
...
double EmModel::gaGetUpdatedLearningRate(double learning_rate, int iter) const {
  ...
  if (USE_DEFAULT_DECAY == cfg->ga_decay_method)
    ...
    learning_rate *=
        pow(cfg->step_decay_drop, floor(iter / cfg->step_decay_epochs_drop));
...
int EmModel::computeAndAccumulateGradient(float *grads, int mol_idx,
  ...
  if (sampling_method != USE_NO_SAMPLING) {
    getSubSampledTransitions(mol_data, sampling_method, energy,
                             selected_trans_id);
  ...
  for (int from_idx = 0; frag_trans_map != mol_data.getFromIdTMap()->end();
       ++frag_trans_map, from_idx++) {
    ...
    std::vector<double> sum_terms(
        mol_data.getFeatureVectorForIdx(0)->getTotalLength());
...
void EmModel::getSubSampledTransitions(MolData &moldata, int sampling_method,
                                       unsigned int energy,
                                       std::set<int> &selected_trans_id) const {

  switch (sampling_method) {
  case USE_RANDOM_SAMPLING: {
    moldata.getRandomSampledTransitions(selected_trans_id,
                                        cfg->ga_sampling_max_selection);
    break;
  }
  case USE_GRAPH_RANDOM_WALK_SAMPLING: {
    moldata.getSampledTransitionIdsRandomWalk(selected_trans_id,
                                              cfg->ga_sampling_max_selection);
    break;
  }
  ...
  case USE_DIFFERENCE_SAMPLING_BFS: {
    ...
    moldata.computePredictedSpectra(*param, true, energy, 1, 1000, 100, 0.0, -1,
                                    cfg->use_log_scale_peak);
    ...
    if (sampling_method == USE_DIFFERENCE_SAMPLING_BFS_CO)
      moldata.getSampledTransitionIdUsingDiffMapBFS(selected_trans_id,
                                                    selected_weights);
    else if (sampling_method == USE_DIFFERENCE_SAMPLING_BFS)
      moldata.getSampledTransitionIdUsingDiffMapCA(selected_trans_id,
                                                   selected_weights);
    break;
  }
  default:
...
double EmModel::computeLogLikelihoodLoss(int molidx, MolData &moldata,
                                         suft_counts_t &suft,
                                         unsigned int energy) {
  ...
  for (int from_idx = 0; it != moldata.getFromIdTMap()->end();
       ++it, from_idx++) {
