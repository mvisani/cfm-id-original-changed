...
void FragmentTreeNode::generateChildrenOfBreak(Break &brk) {
  ...
  int f0_max_limit =
      std::min(total_free_epairs, orig_epairs.first + MAX_E_MOVE);
  int f1_max_limit =
      std::min(total_free_epairs, orig_epairs.second + MAX_E_MOVE);
  ...
  for (auto &rearrangement_config : rearrangement_configs) {
    ...
    for (unsigned int i = 0; i < num_bonds; i++) {
      if (f0_output_bmax[i] >= 1)
        f0_output_bmax[i] -= 1;
    }

    // Iterate through the possible solutions, adding them to the node as
    // children
...
int FragmentTreeNode::addChild(int e_f0, int e_to_allocate,
                               std::vector<int> &output_bmax, Break &brk,
                               int charge_frag) {
  ...
  for (int i = 0; i < numbonds; i++) {
    ...
    if (broken) {
      // Save the bond to remove at the end, removing it now causes problems
      // with the iteration/indexing
    ...
    //           bond->getEndAtomIdx() << " [remaining_e] " <<
    //           remaining_e[fragidx] << " output_bmax[i] " << output_bmax[i] <<
    //           std::endl;
    int num_to_add = 0;
  ...
  // If there's still remaining_e, allocate as lp bond (only use lp bond if
  // necessary)
  if (remaining_e[0] > 0 || remaining_e[1] > 0) {
    for (int i = 0; i < numbonds; i++) {
      ...
      if (output_bmax[i + numbonds] > 0 && remaining_e[fragidx] > 0) {
        bond->setBondType(
            RDKit::Bond::BondType(1 + (int)(bond->getBondTypeAsDouble())));
  ...
  for (unsigned int i = 0; i < rwmol.getNumAtoms(); i++) {
    ...
    // Ionic fragments must retain their charge and not attract hydrogens (for
    // simplicity)
    int ionic_frag_q;
    ...
    // std::cout << "[DEBUG][ATOM][ID " <<  atom->getIdx() << "][" <<
    // atom->getSymbol() <<
    //           "][Charge " << atom->getFormalCharge() << "][OrigValence " <<
    //           orig_val << "]" << std::endl;
  ...
  // std::cout << "[DEBUG][Fill Hydrogensl]" << RDKit::MolToSmiles( rwmol ) <<
  // std::endl;
  ...
  if ((split_charge_fragidx == (1 - charge_frag) && nl_ionic_q == 0) ||
      nl_ionic_q > 0) {
    bool qloc_found = false;
    for (unsigned int i = 0; i < rwmol.getNumAtoms(); i++) {
      RDKit::Atom *atom = rwmol.getAtomWithIdx(i);
      atom->getProp("FragIdx", fragidx);
      if (fragidx == (1 - charge_frag) && atom->getFormalCharge() == 0 &&
  ...
  // std::cout << "[DEBUG][before sanitizeMol]" << RDKit::MolToSmiles( rwmol )
  // << std::endl;
  RDKit::MolOps::sanitizeMol(rwmol);
  // std::cout << "[DEBUG][after sanitizeMol]" << RDKit::MolToSmiles( rwmol ) <<
  // std::endl;
  ...
  for (int radical_frag = min_radical_frag; radical_frag <= max_radical_frag;
       radical_frag++) {
    ...
    // Check for an already charge molecule (e.g. due to N lone pair bonding),
    // or a molecule with split charge (in which case, assign charge there)
    ...
    boost::tuple<bool, bool, bool> qfound_oktogo_switchq =
        findAlreadyChargedOrSplitCharge(pidx_nidx_ridx, rwmol, charge_frag,
                                        radical_frag, is_negative);
    if (boost::get<0>(qfound_oktogo_switchq) &&
        !boost::get<1>(qfound_oktogo_switchq))
      continue;
    if (!boost::get<0>(qfound_oktogo_switchq) ||
        boost::get<2>(qfound_oktogo_switchq)) {

      // Assign the charge (and radical) location
      if (boost::get<2>(qfound_oktogo_switchq))
        qidx_ridx = findChargeLocation(rwmol, 1 - charge_frag, radical_frag,
                                       is_negative);
      else
        qidx_ridx =
            findChargeLocation(rwmol, charge_frag, radical_frag, is_negative);
      if (qidx_ridx.first < 0)
        continue; // If we can't find a charge location, don't include this one.
      if (is_radical && qidx_ridx.second < 0)
        continue; // If we can't find a radical location (if needed), don't
                  // include this one.
      assignChargeAndRadical(rwmol, qidx_ridx.first, qidx_ridx.second,
                             is_negative);
    }
    ...
    if (brk.isRingBreak()) {
      ...
      // TODO: NOT the smartest way to do this, we need better way to handle
      // ring break and multi frags
      ...
      for (ai = child_nl.get()->beginAtoms(); ai != child_nl.get()->endAtoms();
           ++ai) {
      ...
      auto child_node =
          FragmentTreeNode(child_ion, child_nl, allocated_e[charge_frag],
                           depth + 1, fh, child_e_loc, true);
      ...
      if (!brk.isCycliaztion()) {
        ...
        // std::cout << "[DEBUG][Child Ion]" << RDKit::MolToSmiles( *child_ion )
        // << std::endl;
        labelExtraBreakPropertiesInIon(child_ion, ion, brk);
        auto child_node =
            FragmentTreeNode(child_ion, child_nl, allocated_e[charge_frag],
        ...
        number_child_added++;
        /*
        for (auto ai = child_ion->beginAtoms(); ai != child_ion->endAtoms();
        ++ai) { int is_current = 0;
        ...
        for (auto ai = rw_child_ion.beginAtoms(); ai != rw_child_ion.endAtoms();
             ++ai) {
          int is_ring_root = 0;
        ...
        // get distance matrix
        double *dist_matrix =
            RDKit::MolOps::getDistanceMat(rw_child_ion, false, false);
        int num_atoms = rw_child_ion.getNumAtoms();
        int distance =
            dist_matrix[ring_root_atom_idx * num_atoms + current_root_atom_idx];

        if (distance > 3 && distance < 6 && (num_h_ring_root > 1) &&
            (num_h_current_root > 1)) {

          // add a single bond between atom
          rw_child_ion.addBond(*ring_root_ai, *current_root_ai,
          ...
          auto bond = rw_child_ion.getBondBetweenAtoms(ring_root_atom_idx,
                                                       current_root_atom_idx);
          ...
          romol_ptr_t cyclizated_child_ion =
              boost::make_shared<RDKit::ROMol>(RDKit::ROMol(rw_child_ion));
          createChildIonElectronLocRecord(child_e_loc, cyclizated_child_ion);
          labelExtraBreakPropertiesInIon(child_ion, ion, brk);
          auto child_node = FragmentTreeNode(cyclizated_child_ion, child_nl,
                                             allocated_e[charge_frag],
                                             depth + 1, fh, child_e_loc, false);
      ...
      for (int mol_idx = 0; mol_idx < mols.size(); mol_idx++) {
        ...
        if (set_flags[fragidx])
          acc_mols[fragidx].reset(
              RDKit::combineMols(*acc_mols[fragidx], *mols[mol_idx]));
        else {
    ...
    if (!boost::get<0>(qfound_oktogo_switchq) ||
        boost::get<2>(qfound_oktogo_switchq))
      undoChargeAndRadical(rwmol, qidx_ridx.first, qidx_ridx.second,
                           is_negative);
    else //(Split charge will still require undo)
      undoAlreadyChargedOrSplitCharge(rwmol, pidx_nidx_ridx);
  }

  // For Hydrogen loss only, remove the added hydrogens (should be just the last
  // atom, but doesn't assume...)
  if (brk.getBondIdx() == -1 && !brk.isRingBreak()) {
...
std::pair<int, int> FragmentTreeNode::findChargeLocation(RDKit::RWMol &rwmol,
                                                         int charge_side,
                                                         int radical_side,
                                                         bool is_negative) {
  ...
  // Find a non-ring Nitrogen, Sulfur, Oxygen, Carbon, or a ring N,S,O,C
  // (preferenced in that order)
  ...
  if (radical_side == charge_side) {
    ...
    // For non-C charge locations, we will remove one electron from that atom to
    // create the charged radical
    ...
    for (bi = rwmol.beginBonds(); bi != rwmol.endBonds(); ++bi) {
      //(Note: We assume here that any broken bonds have been removed beforehand
      //so we don't check for broken-ness).
      int fragidx;
      (*bi)->getBeginAtom()->getProp("FragIdx", fragidx);
      if (fragidx != charge_side)
        continue;
    ...
    // Allocate the radical somewhere on the neutral loss (anywhere with a H to
    // remove)
...
FragmentTreeNode::findAlreadyChargedOrSplitCharge(
    boost::tuple<int, int, int> &out_pidx_nidx_ridx, RDKit::RWMol &rwmol,
  ...
  // Disallow molecules with 2+ total charge -> can otherwise occur if lone pair
  // and ionic occur together.
  //  std::cout <<"[DEBUG][findAlreadyChargedOrSplitCharge]"
  //  <<RDKit::MolOps::getFormalCharge(rwmol) << std::endl;
  ...
  for (ai = rwmol.beginAtoms(); ai != rwmol.endAtoms(); ++ai) {
    ...
    if (q < 0)
      neg_idxs.push_back((*ai)->getIdx());
    if ((*ai)->getNumRadicalElectrons() == 1)
      rad_idx = (*ai)->getIdx(); // Note: only an assigned rad idx is added to
                                 // out_pidx_nidx_ridx, this one is existing.
  }

  // std::cout <<"[DEBUG][findAlreadyChargedOrSplitCharge][charge_side]" <<
  // frag_total_qs[charge_side] << "[other_side]" << frag_total_qs[1 -
  // charge_side] << std::endl;
  ...
  if (!is_negative && frag_total_qs[charge_side] == 1 &&
      frag_total_qs[1 - charge_side] == 0) {
    if (radical_side < 0 && rad_idx == -1)
      boost::get<1>(qfound_oktogo_switchq) =
          true; // No radical required or found
    else if (radical_side >= 0 && rad_idx >= 0) {
      int rfragidx;
      rwmol.getAtomWithIdx(rad_idx)->getProp("FragIdx", rfragidx);
      if (rfragidx == radical_side)
  ...
  if (!is_negative && frag_total_qs[charge_side] == 1 &&
      frag_total_qs[1 - charge_side] == -1) {

    // Find a non-ionic negative idx on the neutral loss to add a charge/radical
    // to
    ...
    // If that didn't work - prepare to try adding the charge on the neutral
    // loss side instead of the ion
...
void FragmentTreeNode::undoAlreadyChargedOrSplitCharge(
    RDKit::RWMol &rwmol, boost::tuple<int, int, int> &pidx_nidx_ridx) {
...
int FragmentTreeNode::findAtomChargeLocationNSOC(RDKit::RWMol &rwmol,
                                                 int charge_side,
                                                 bool is_negative) {
  ...
  for (unsigned int i = 0; i < rwmol.getNumAtoms(); i++) {
    ...
    if (fragidx != charge_side)
      continue;
    if (is_negative && atom->getTotalNumHs() == 0)
      continue;
    ...
    if (numrings > 0) {
      ...
      if (atom->getSymbol() == "N" && Nidx < 0)
        ...
        Oidx = i;
               ...
               (atom->getSymbol() == "Cl" || atom->getSymbol() == "I" ||
...
void FragmentTreeNode::assignChargeAndRadical(RDKit::RWMol &rwmol,
                                              int charge_idx, int radical_idx,
                                              bool is_negative) {

  RDKit::Atom *atom = rwmol.getAtomWithIdx(charge_idx);
  // This is important, CC[CH3+] and CC[CH+] has the same radical electrons and
  // formal charges, set H for charged atom  to avoid this problem
  alterNumHs(atom, 0);
  if (is_negative) {
    atom->setFormalCharge(-1);
    ...
    if (radical_idx < 0)
      ...
      if (radical_idx !=
          charge_idx) { // Else Odd+. -> Odd+. (lone pair or C.H) + NL
        int rad_fragidx;
        ...
        if (rad_fragidx != charge_fragidx) { // Odd+. -> Even+[H+] + NL-[H.]
          ...
          RDKit::Bond *mult_bond =
              rwmol.getBondBetweenAtoms(radical_idx, charge_idx);
          mult_bond->setBondType(RDKit::Bond::BondType(
              (int)(mult_bond->getBondTypeAsDouble() - 1.0)));
...
void FragmentTreeNode::undoChargeAndRadical(RDKit::RWMol &rwmol, int charge_idx,
...
void FragmentTreeNode::labelBreakPropertiesInNL(romol_ptr_t &current_nl,
                                                romol_ptr_t &parent_ion,
                                                Break &brk) {
  ...
  if (fh->getExecFlag(3)) {
    ...
    if (brk.isIonicBreak())
      ...
      RDKit::Bond *brokenbond =
          parent_ion.get()->getBondWithIdx(brk.getBondIdx());
      int bondtype;
...
void FragmentTreeNode::labelExtraBreakPropertiesInIon(romol_ptr_t &current_ion,
                                                      romol_ptr_t &parent_ion,
                                                      Break &brk) {
...
void FragmentTreeNode::generateBreaks(std::vector<Break> &breaks,
                                      bool include_H_only_loss,
                                      bool include_cyclization) {

  int num_ionic = countNumIonicFragments(ion.get());
  RDKit::PeriodicTable *pt = RDKit::PeriodicTable::getTable();

  // Populate the Ring Info for the ion and set the NumUnbrokenRings and
  // OrigValence properties
  RDKit::MolOps::findSSSR(*ion.get());
  RDKit::RingInfo *rinfo = ion.get()->getRingInfo();
  RDKit::ROMol::AtomIterator ai;
  // std::cout << "[DEBUG][generateBreaks]" << RDKit::MolToSmiles( *ion.get() )
  // << std::endl;
  ...
  for (ai = ion.get()->beginAtoms(); ai != ion.get()->endAtoms(); ++ai) {
    ...
    // Fetch or compute the valence of the atom in the input molecule (we
    // disallow valence changes for now)
    auto orig_val = getValence(*ai);
    (*ai)->setProp("OrigValence", orig_val);
    (*ai)->setProp("Root", 0);
    (*ai)->setProp("OtherRoot", 0);

    // Create breaks for any implied ionic bonds (-1 bond_idx, and ring_idx
    // overloaded with the atom idx)
    int ionic_frag_q;
    (*ai)->getProp("IonicFragmentCharge", ionic_frag_q);
    if (ionic_frag_q != 0 &&
        num_ionic !=
            ion.get()->getNumAtoms()) //(must have at least one non-ionic atom
                                      //to break further)
      breaks.push_back(Break((*ai)->getIdx(), true, false, -1,
                             computeNumIonicAlloc(num_ionic - 1), false));
  }
  ...
  for (unsigned int bidx = 0; bidx < ion.get()->getNumBonds(); bidx++) {
    ...
    if (rinfo->numBondRings(bidx) == 0)
      breaks.push_back(Break(bidx, false, false, -1,
                             computeNumIonicAlloc(num_ionic), false));
    if (was_on_the_ring && include_cyclization)
      // add a cyclization break
      breaks.push_back(
          Break(bidx, false, false, -1, computeNumIonicAlloc(num_ionic), true));
    else
      ring_bonds_count++;
  }
  ...
  if (had_ring_break == 0) {
    ...
    for (int ring_idx = 0; bit != brings.end(); ++bit, ring_idx++) {
      ...
      //(larger rings can exist, but will not break since it blows out the
      //computation)
      ...
      for (auto it = bit->begin(); it != bit->end(); ++it) {
        ...
        if (rinfo->numBondRings(*it) != 1)
          continue;
        breaks.push_back(Break(*it, false, true, ring_idx,
                               computeNumIonicAlloc(num_ionic), false));
      }
    }
  }
...
void FragmentTreeNode::applyBreak(Break &brk, int ionic_allocation_idx) {
  ...
  if (brk.isHydrogenOnlyBreak()) {
    // Hydrogen Only Break - No explicit bond to break, set arbitrary root atom
    // (pick a C if there is one)
    int root_idx = 0;
    for (ai = ion.get()->beginAtoms(); ai != ion.get()->endAtoms(); ++ai)
      if ((*ai)->getSymbol() == "C") {
        root_idx = (*ai)->getIdx();
    ...
    for (ai = ion.get()->beginAtoms(); ai != ion.get()->endAtoms(); ++ai) {
      ...
      if (a_ionic_q != 0.0)
        continue; // Don't pick another ionic fragment
      if ((*ai)->getFormalCharge() == -ionic_q) {
        root_idx = (*ai)->getIdx();
        break;
      }
      if (root_idx < 0 && (*ai)->getIdx() != brk.getIonicIdx())
        root_idx = (*ai)->getIdx();
    }
    ion.get()->getAtomWithIdx(root_idx)->setProp("Root", 1);
  ...
  // Assign fragment indexes for any (other) ionic fragments (which are
  // otherwise always FragIdx=0)
  int ionic_idx = ionic_allocation_idx;
  for (ai = ion.get()->beginAtoms();
       ai != ion.get()->endAtoms() && ionic_idx > 0; ++ai) {
    int ionic_q;
    (*ai)->getProp("IonicFragmentCharge", ionic_q);
    if (ionic_q != 0 &&
        (!brk.isIonicBreak() || (*ai)->getIdx() != brk.getIonicIdx())) {
      if (ionic_idx & 0x1)
...
void FragmentTreeNode::undoBreak(Break &brk, int ionic_allocation_idx) {
  ...
  if (brk.isHydrogenOnlyBreak()) {
    ...
    // Pick a root atom on the other fragment (ideally one of opposite charge to
    // the ionic atom, else any)
    int root_idx = -1;
    RDKit::ROMol::AtomIterator ai;
    for (ai = ion.get()->beginAtoms(); ai != ion.get()->endAtoms(); ++ai) {
      if ((*ai)->getFormalCharge() == -ionic_q) {
  ...
  // Un-assign fragment indexes for any ionic fragments (which are otherwise
  // always FragIdx=0)
  int ionic_idx = ionic_allocation_idx;
  RDKit::ROMol::AtomIterator ai;
  for (ai = ion.get()->beginAtoms();
...
void FragmentTreeNode::allocatedCtdToFragment(RDKit::ROMol *romol,
                                              RDKit::Atom *atom) {

  // set fragidx to 1 until we meet the broken bone
  // this works if there is only two fragments since all fragidx already set to
  // 1
  int broken, fragidx;
  atom->setProp("FragIdx", 1);
  ...
  for (; itp.first != itp.second; ++itp.first) {
    ...
    RDKit::Bond *bond =
        romol->getBondBetweenAtoms(atom->getIdx(), nbr_atom->getIdx());
    bond->getProp("Broken", broken);
    if (!fragidx && !broken)
      allocatedCtdToFragment(romol, nbr_atom);
  }
}
...
int FragmentTreeNode::countNumIonicFragments(const RDKit::ROMol *romol) {
  int num_ionic = 0;
  for (RDKit::ROMol::ConstAtomIterator ait = romol->beginAtoms();
       ait != romol->endAtoms(); ++ait) {
    int ionic_frag_q;
...
void FragmentTreeNode::createChildIonElectronLocRecord(
    std::vector<int> &child_e_loc, romol_ptr_t childmol) {
