...
void EM::infCheck(double &belief, MolData *moldata, std::string str) {
    // TODO FIND A BETTER WAY THIS IS A SUPER HACKY FIX
    if (boost::math::isinf(belief)) {
        if (belief < 0.0) {
            std::cerr << str << " Warning: belief is -Inf " << moldata->getId() << std::endl;
            //belief = -100000000000;
        } else {
            std::cerr << str << " Warning: belief is Inf " << moldata->getId() <<  std::endl;
            //belief = 100000000000;
