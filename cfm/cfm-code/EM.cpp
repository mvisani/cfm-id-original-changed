...
double EM::run(std::vector<MolData> &data, int group, std::string &out_param_filename) {
    ...
    while (iter < MAX_EM_ITERATIONS) {
        ...
        std::string estep_time_msg =
                "Completed E-step processing: Time Elapsed = " + boost::lexical_cast<std::string>(after - before) +
                " seconds";
        ...
        std::string debug_time_msg =
                "Starting Q compute: Time Elapsed = " + boost::lexical_cast<std::string>(after - before) + " seconds";
        ...
        std::string debug_time_msg2 =
                "Finished Q compute: Time Elapsed = " + boost::lexical_cast<std::string>(after - before) + " seconds";
        ...
        if (comm->isMaster()) {
            std::string qdif_str =
                    boost::lexical_cast<std::string>(Qratio) + " " + boost::lexical_cast<std::string>(prevQ) + " ";
            qdif_str += "Q=" + boost::lexical_cast<std::string>(Q) + " ValQ=" + boost::lexical_cast<std::string>(valQ) +
                        " ";
            qdif_str += "Qn=" + boost::lexical_cast<std::string>(Q / numnonvalmols) + " ValQn=" +
                        boost::lexical_cast<std::string>(valQ / numvalmols) + " ";
        ...
        if (Qratio < cfg->em_converge_thresh || count_no_progress >= 3) {
            comm->printToMasterOnly(
                    ("EM Converged after " + boost::lexical_cast<std::string>(iter) + " iterations").c_str());
            break;
        }
        iter++;
    }
    ...
    if (iter >= MAX_EM_ITERATIONS)
        comm->printToMasterOnly(("Warning: EM did not converge after " + boost::lexical_cast<std::string>(iter) +
                                 " iterations.").c_str());
...
lbfgs_evaluate(void *instance, const lbfgsfloatval_t *x, lbfgsfloatval_t *g, const int n, const lbfgsfloatval_t step) {
    lbfgsfloatval_t fx = ((EM *) instance)->evaluateLBFGS(x, g, n, step);
    return fx;
}

static int lbfgs_progress(void *instance, const lbfgsfloatval_t *x, const lbfgsfloatval_t *g, const lbfgsfloatval_t fx,
...
lbfgsfloatval_t
EM::evaluateLBFGS(const lbfgsfloatval_t *x, lbfgsfloatval_t *g, const int n, const lbfgsfloatval_t step) {
...
void EM::progressLBFGS(const lbfgsfloatval_t *x, const lbfgsfloatval_t *g, const lbfgsfloatval_t fx,
                       const lbfgsfloatval_t xnorm, const lbfgsfloatval_t gnorm, const lbfgsfloatval_t step, int n,
                       int k, int ls) {

    if (comm->isMaster()) {
        writeStatus(("LBFGS Iteration " + boost::lexical_cast<std::string>(k) + ": fx = " +
                     boost::lexical_cast<std::string>(fx)).c_str());
...
double EM::updateParametersSimpleGradientDescent(std::vector<MolData> &data, suft_counts_t &suft) {
    ...
    while (iter++ < cfg->ga_max_iterations && fabs((Q - prev_Q) / Q) >= cfg->ga_converge_thresh) {
        ...
        //Step the parameters
        if (comm->isMaster())
            param->adjustWeightsByGrads(grads, ((MasterComms *) comm)->master_used_idxs, learn_rate, cfg->ga_momentum,
                                        prev_v);
        comm->broadcastParams(param.get());
