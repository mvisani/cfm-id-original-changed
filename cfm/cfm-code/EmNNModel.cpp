...
#				model parameters when using a neural net for
theta. #				- all identical to linear model, except
params are NNParam and gradient #				  computation is
different. #					E-step: IPFP or equivalent. #
...
EmNNModel::EmNNModel(config_t *a_cfg, FeatureCalculator *an_fc,
  ...
  if (!initial_params_provided) {
    nn_param = boost::shared_ptr<NNParam>(new NNParam(
        fc->getFeatureNames(), num_energies_to_include,
        a_cfg->theta_nn_hlayer_num_nodes, a_cfg->theta_nn_layer_act_func_ids,
        a_cfg->nn_layer_dropout_probs, a_cfg->nn_layer_is_frozen_flags));
    ...
    std::string msg =
        "EM_NN: Initial params provided from " + initial_params_filename;
    std::cout << msg << std::endl;
  }
  this->param = nn_param;
}

// These functions are the same as for EM, but use NNParam, so need to be
// redefined here
...
int EmNNModel::computeAndAccumulateGradient(float *grads, int mol_idx,
  ...
  if (!mol_data.hasComputedGraph())
    return static_cast<int>(num_used_transitions);
  ...
  for (int from_idx = 0; from_idx < num_fragments; from_idx++) {
    ...
    // Compute the forward values, storing intermediate a and z values, and the
    // combined denom of the rho term, and
    // Q
    double denom = 1.0;
    ...
    for (int idx = 0; it != from_id_map->end(); ++it, idx++) {
      ...
      double theta = nn_param->computeTheta(*fvs[idx], energy, z_values[idx],
                                            a_values[idx], false, true);
      denom += exp(theta);
      nu_terms[idx] = (*suft_values)[*it + suft_offset];
    }
    nu_terms[num_trans_from_id] =
        (*suft_values)[offset + from_idx + suft_offset];

    // Compute the deltas
    std::vector<azd_vals_t> deltasA, deltasB;
    nn_param->computeDeltas(deltasA, deltasB, z_values, a_values, denom,
                            energy);
    // Compute the unweighted gradients
    std::vector<std::vector<float>> unweighted_grads;
    std::set<unsigned int> from_id_used_idxs;
    nn_param->computeUnweightedGradients(unweighted_grads, from_id_used_idxs,
                                         fvs, deltasA, deltasB, a_values);
    ...
    for (int idx = 0; idx <= num_trans_from_id; idx++) {
      ...
      for (sit = from_id_used_idxs.begin(); sit != from_id_used_idxs.end();
           ++sit)
...
void EmNNModel::collectUsedIdx(MolData &mol_data,
  ...
  for (auto frag_trans_map = mol_data.getFromIdTMap()->begin();
    ...
    for (auto trans_id : *frag_trans_map) {
      ...
      for (auto fv_it = fv->getFeatureBegin(); fv_it != fv->getFeatureEnd();
           ++fv_it)
        nn_param->collectUsedIdx(used_idxs, feature_len, grad_offset, *fv_it,
                                 energy);
    }
  }
...
double EmNNModel::computeLogLikelihoodLoss(int molidx, MolData &moldata,
                                           suft_counts_t &suft,
                                           unsigned int energy) {
...
void EmNNModel::updateGradientForRegularizationTerm(float *grads,
  ...
  // Remove part of the Bias terms (regularize the bias terms 100 times less
