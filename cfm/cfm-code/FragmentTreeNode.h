...
public:
  ...
  Break()
      : h_only_break(true), ring_break(false), ionic_break(false), bond_idx(-1),
        ring_idx(-1), num_ionic_frag_allocations(1), ionic_idx(-1),
        is_cycliaztion(false){};

  // Constructor for Break (ionic or standard or ring)
  Break(int a_bond_or_ionic_atom_idx, bool a_ionic_break, bool is_ring_break,
        int a_ring_idx, int a_num_ionic_frag_allocations, bool is_cycliaztion)
      : h_only_break(false), ring_break(is_ring_break),
        ionic_break(a_ionic_break), bond_idx(-1), ring_idx(-1), ionic_idx(-1),
        num_ionic_frag_allocations(a_num_ionic_frag_allocations),
        is_cycliaztion(is_cycliaztion) {
...
private:
  ...
  // neutral loss
  void labelBreakPropertiesInNL(romol_ptr_t &current_nl,
                                romol_ptr_t &parent_ion, Break &brk);

  // Utility function to store extra  properties of the break in the generated
  // iom
  void labelExtraBreakPropertiesInIon(romol_ptr_t &current_ion,
                                      romol_ptr_t &parent_ion, Break &brk);
