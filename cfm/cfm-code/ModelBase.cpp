...
void ModelBase::setMiniBatchFlags(std::vector<int> &minibatch_flags,
                                  int num_batch) {
...
Solver *ModelBase::getSolver(int ga_method, double learning_rate) const {
  ...
  case USE_ADAM_FOR_GA:
    solver = new Adam(param->getNumWeights(), learning_rate,
                      cfg->ga_adam_beta_1, cfg->ga_adam_beta_2,
                      cfg->ga_adam_eps, cfg->ga_adam_use_amsgrad);

    break;
  case USE_ADAMW_FOR_GA:
    solver =
        new AdamW(param->getNumWeights(), learning_rate, cfg->ga_adam_beta_1,
                  cfg->ga_adam_beta_2, cfg->ga_adam_eps,
                  cfg->ga_adam_use_amsgrad, cfg->ga_adamw_w);
    break;
  case USE_ADABELIEF_FOR_GA:
    solver = new AdaBelief(param->getNumWeights(), learning_rate,
                           cfg->ga_adam_beta_1, cfg->ga_adam_beta_2,
                           cfg->ga_adam_eps, cfg->ga_adam_use_amsgrad);

    break;
  case USE_ADADELTA_FOR_GA:
    solver = new AdaDelta(param->getNumWeights(), learning_rate,
                          cfg->ga_adadelta_rho, cfg->ga_adam_eps);
    break;
  case USE_MOMENTUM_FOR_GA:
    solver =
        new Momentum(param->getNumWeights(), learning_rate, cfg->ga_momentum);
    break;
  case USE_SGD_FOR_GA:
  default:
