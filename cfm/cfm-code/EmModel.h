...
public:
  // Constructor
  // Note: To include the group in the status filename, include _GRP_ in the
  // name
  ...
  double trainModel(std::vector<MolData> &molDataSet, int group,
  ...
  virtual void collectUsedIdx(MolData &mol_data,
                              std::set<unsigned int> &used_idxs,
                              unsigned int energy);

  virtual double computeLogLikelihoodLoss(int molidx, MolData &moldata,
                                          suft_counts_t &suft,
                                          unsigned int energy);
...
protected:
  ...
  void recordSufficientStatistics(suft_counts_t &suft, int molidx,
                                  MolData *moldata, beliefs_t *beliefs,
                                  unsigned int energy);

  // Simple gradient ascent
  double updateParametersGradientAscent(std::vector<MolData> &data,
                                        suft_counts_t &suft,
                                        double learning_rate,
                                        int sampling_method,
                                        unsigned int energy);

  double computeAndSyncLoss(std::vector<MolData> &data, suft_counts_t &suft,
                            unsigned int energy);
  ...
  std::tuple<double, double, double, double> computeMetrics(const Spectrum *p,
                                                            const Spectrum *q);

  double gaGetUpdatedLearningRate(double learning_rate, int iter) const;

  void updateEmTrainingParams(double loss_change_rate, float &learning_rate,
                              int &sampling_method,
  ...
  static float
  getTimeDifference(const std::chrono::system_clock::time_point &before,
                    const std::chrono::system_clock::time_point &after);

  static std::string
  getTimeDifferenceStr(const std::chrono::system_clock::time_point &before,
                       const std::chrono::system_clock::time_point &after);
};
