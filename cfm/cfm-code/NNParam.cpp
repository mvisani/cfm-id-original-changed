...
NNParam::NNParam(std::vector<std::string> a_feature_list,
                 int a_num_energy_levels, std::vector<int> &a_hlayer_num_nodes,
  ...
  // The weight length was set in the Param constructor, now alter it to match
  // the neural net layout
  ...
  total_len += num_input_layer_weights; // The first layer takes the fv as input
                                        // (which has in-built bias).
  ...
  // Set pointers to the activation functions and their derivatives used in each
  // layer
  setActivationFunctionsFromIds();

  // Resize some temporary a and z vectors so we don't have to
  //  allocate memory for a and z within the computeTheta function if we don't
  //  need them after.
...
void NNParam::randomUniformInit() {
  ...
  for (unsigned int i = 0; i < weights.size(); i++)
    weights[i] = distribution(
        util_rng); //(float(std::rand()) / float(RAND_MAX) - 0.5) * 0.2;
}
...
void NNParam::randomNormalInit() {
  ...
  for (unsigned int energy_level_idx = 0;
       energy_level_idx < getNumEnergyLevels(); energy_level_idx++) {
    int weight_offset = 0;
...
void NNParam::varianceScalingInit() {
  ...
  for (unsigned int energy_level_idx = 0;
       energy_level_idx < getNumEnergyLevels(); energy_level_idx++) {
    int weight_offset = 0;
    for (int hlayer_idx = 0; hlayer_idx < h_layer_num_nodes.size();
         ++hlayer_idx) {
...
float NNParam::computeTheta(const FeatureVector &fv, int energy,
  ...
  // The first hidden layer takes the fv as input (which already has a bias
  // feature,
  ...
  for (int h_node = 0; h_node < (*layer_it); h_node++) {
    if (!use_dropout || !hlayer_is_dropped[neuron_idx]) {
      ...
      for (auto feature_it = fv.getFeatureBegin();
           feature_it != fv.getFeatureEnd(); ++feature_it)
      ...
      if (use_dropout)
        a_values[neuron_idx] =
            a_values[neuron_idx] / (1.0 - hlayer_dropout_probs[0]);
  ...
  for (int h_layer_idx = 1; h_layer_idx < h_layer_num_nodes.size();
       ++h_layer_idx) {
    for (int h_node_idx = 0; h_node_idx < h_layer_num_nodes[h_layer_idx];
         ++h_node_idx) {
      if (!use_dropout || !hlayer_is_dropped[neuron_idx]) {
        ...
        if (use_dropout)
          a_values[neuron_idx] =
              a_values[neuron_idx] / (1.0 - hlayer_dropout_probs[h_layer_idx]);
...
NNParam::NNParam(std::string &filename) : Param(filename) {
  ...
  // We've already read all the weights, here we want to read the neural net
  // parameters, which should be appended right at the end of the file
  std::string line;
  ...
  if (!found_nn_details)
    throw NNParamFileReadException();
}
...
void NNParam::computeDeltas(std::vector<azd_vals_t> &deltasA,
                            std::vector<azd_vals_t> &deltasB,
                            std::vector<azd_vals_t> &z_values,
                            std::vector<azd_vals_t> &a_values, float rho_denom,
                            int energy) {
  ...
  // Set up the iterators (we are going backwards through the network, so set up
  // iterators in reverse)
  std::vector<azd_vals_t::reverse_iterator> itAs(num_trans_from_id),
      itBs(num_trans_from_id);
  std::vector<azd_vals_t::reverse_iterator> aits(num_trans_from_id),
      zits(num_trans_from_id);
  for (int idx = 0; idx < num_trans_from_id; idx++) {
  ...
  std::vector<azd_vals_t::reverse_iterator> itAs_input = itAs,
                                            itBs_input = itBs;

  int energy_offset =
      (num_energy_levels - energy - 1) * getNumWeightsPerEnergyLevel();
...
void NNParam::computeUnweightedGradients(
  ...
  unweighted_grads.resize(num_trans_from_id +
                          1); //+1 for the persistence transition (stored last)
  std::vector<std::vector<float>::iterator> itgrads(num_trans_from_id),
      itAs(num_trans_from_id), itBs(num_trans_from_id), aits(num_trans_from_id);
  for (int idx = 0; idx < num_trans_from_id; idx++) {
  ...
  unweighted_grads[num_trans_from_id].resize(getNumWeightsPerEnergyLevel(),
                                             0.0f);

  // If there's no transitions from this id except persistence, the unweighted
  // gradients for that persistence transition will all be 0
  if (num_trans_from_id == 0)
    return;

  std::vector<float>::iterator normit =
      unweighted_grads[num_trans_from_id].begin();
  unsigned int feature_len = fvs[0]->getTotalLength();

  // Collect the used feature idxs (we'll need these for the first layer
  // normalization)
  ...
  if (!froze_layer) {
    for (int idx = 0; idx < num_trans_from_id; idx++) {
      for (auto fit = fvs[idx]->getFeatureBegin();
           fit != fvs[idx]->getFeatureEnd(); ++fit) {
  ...
  for (int hnode = 0; hnode < *itlayer; hnode++) {
    // Compute the unnormalized terms, tracking the normalizers (persistence
    // terms) as we go
    if (!froze_layer) {
      for (int idx = 0; idx < num_trans_from_id; idx++) {
      ...
      // (Note: we need to normalize for all transitions if any transition used
      // that feature)
      for (int idx = 0; idx < num_trans_from_id; idx++) {
...
void NNParam::rollDropouts() {
  ...
  for (int hlayer_idx = 0; hlayer_idx < h_layer_num_nodes.size();
    ...
    for (int h_node_idx = 0; h_node_idx < h_layer_num_nodes[hlayer_idx];
         ++h_node_idx) {
      indice.push_back(neuron_idx);
      neuron_idx++;
    }

    auto expected_deactive_count =
        (unsigned int)std::floor(drop_ratio * h_layer_num_nodes[hlayer_idx]);
