...
* CFM-ID MSRB (mass spectrum rule based) can be found at ([https://bitbucket.org/wishartlab/msrb-fragmenter/src/master/](https://bitbucket.org/wishartlab/msrb-fragmenter/src/master/))
* Original source code from CFM-ID 2.0 Can be found at ([https://sourceforge.net/p/cfm-id/wiki/Home/](https://sourceforge.net/p/cfm-id/wiki/Home/))
...
Docker Image can be found at [https://hub.docker.com/r/wishartlab/cfmid](https://hub.docker.com/r/wishartlab/cfmid).
...
* **smiles_or_inchi_or_file**: The smiles or inchi string of the structure whose spectra you want to predict. Or alternatively a .txt file containing a list of space-separated (id, smiles_or_inchi) pairs one per line. e.g.
  ...
  Molecule1 CCCNNNC(O)OMolecule2 InChI=1S/C8H10N4O2/c1-10-4-9-6-5(10)7(13)12(3)8(14)11(6)2/h4H,1-3H3
...
For now ,please refer to [https://sourceforge.net/p/cfm-id/wiki/Home/](https://sourceforge.net/p/cfm-id/wiki/Home/)
...
```bash
docker build -t cfmid:lastest -f .\docker\CFMID4.dockerfile .
```
...
```bash
docker build -t cfmid:v2 -f .\docker\CFMID2and3.dockerfile .
```
...
```bash
docker build -t cfmid:dev-f .\docker\DevBuild.dockerfile .
```
